...
def compute_objective(weights,expected_returns,covariance,rf=0.0452):
    portfolio_returns = weights @ expected_returns 
    portfolio_variance = weights @ covariance @ weights
    risk = np.sqrt(portfolio_variance)
    sharpe = (portfolio_returns - rf) / risk # Optimisation objective function for tangency portfolio
...
sol = sp.minimize(compute_objective,x0,args=(expected_returns,covariance),method='SLSQP',bounds=bounds,constraints=cons)

weights = sol.x

# Maximum sharpe ratio portfolio
portfolio_return_max_sharpe,portfolio_risk_max_sharpe,sharpe_max = generate_portfolio(weights=weights,expected_returns=expected_returns,covariance=covariance)

# Print statements
print(f"------Optimal Weights (Max Sharpe)------")
for ticker,weight in zip(tickers,weights):
    print(f"{ticker}: {weight:.2f}")
print(f"Portfolio Return (%): {portfolio_return_max_sharpe*100:.2f}")
print(f"Porfolio Risk (σ): {portfolio_risk_max_sharpe:.2f}")
...
fig.add_trace(
    go.Scatter(
        ...
        mode="markers",name=f"Maximum Sharpe Ratio Portfolio {sharpe_max:.2f}",
        marker=dict(
        size=10)
        )
    )
